...
boolean turnSquareman = false, turnPolywoman = false; //are they horizontally flipped from their original orientation?
...
void drawSquareFeet(float[], float[]);
...
void setSquareFeetTransform(void);
...
int main(int argc, char* *argv) {
	//window header name
	char header[] = "Homework 3: Squareman Cutscene by Russell Ferguson";
...
void RenderScene(void) {
	...
	setSquareFeetTransform();
	drawSquareFeet(sqMx, sqMy);
	setSqSwordTransform();
...
void drawSquare(float sqMx[], float sqMy[]) {
	int i;
	//squareman body
	glColor3f(0.0, 1.0, 0.0);	//the villain's green colored!
	glShadeModel(GL_FLAT);
	glBegin(GL_POLYGON);
	//draw top half body
	for (i = 0; i <= 3; i++) {
...
void drawSquareFeet(float sqMx[], float sqMy[]) {
...
void setManFeetTransform(void) {
	...
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();
	glTranslatef(dx + dxWalkM, dy + dyWalkM, 0.0);
	glRotatef(theta, 0.0, 0.0, 1.0);
	return;
}
...
void setWomanFeetTransform(void) {
	...
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();
	glTranslatef(dxW + dxWalkW, dyW + dyWalkW, 0.0);
	glRotatef(thetaW, 0.0, 0.0, 1.0);
}

//transformation for Squareman
void setSquareTransform(void) {
...
void setSquareFeetTransform(void) {
	...
	cout << "setSquareFeetTransform" << endl;
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();
	glTranslatef(dxSq + dxWalkS, dySq + dyWalkS, 0.0);
	glRotatef(thetaSq, 0.0, 0.0, 1.0);
}
...
void squareStepping() {
	if (walkSquareman) {
		...
		else if (dyWalkS >= dySq + 3.0) {
			dyWalkS -= 0.1;	//step down
		}
	}
}
...
void timerFunction(int value) { //animation handled here
	...
	case 9: //STAB, polywoman stops smiling
		dxSqSword += 0.15;
		allStepping();
		if (dxSqSword >= 2.0) { dxSqSword = 2.0; frame = 10; polywomanMouth = false; walkPolyman = false; }
		break;
