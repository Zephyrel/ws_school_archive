...
int main(int argc, char* *argv) {
	//window header name
	char header[] = "Homework 5: Squareman Cutscene by Russell Ferguson";
...
void timerFunction(int value) { //animation handled here
	...
	case 5: //polywoman nods her head
		thetaW -= 5.0;
		allStepping();
		if (thetaW <= -60.0) { thetaW = -60.0; frame = 6; }
		break;
	case 6: //polywoman turns her head up from the bow, makes polyman smile bigger
		thetaW += 5.0;
		allStepping();
		if (thetaW >= 0.0) { thetaW = 0; frame = 7; polymanMouth = true; turnPolywoman = true; }
		break;
