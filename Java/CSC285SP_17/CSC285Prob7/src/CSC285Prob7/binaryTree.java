...
public class binaryTree<T extends Comparable<T>> {
	...
	public void insert(Node<T> rootNode, T t){
		...
		}else if(t.compareTo(rootNode.data)==0)System.out.println("ERROR: HEY. NO DUPLICATES ALLOWED IN A BST, PUNK!");
	}
	//Add. DONE!
	public void add(T t){
		if(root==null){
	...
	public void printAll(){
		LinkManager<T> list = new LinkManager<T>();
		Iterator<T> ptr=list.iterator();
		list=this.inOrder(this.root);
		ptr=list.iterator();
		System.out.println("InOrder:");
		while(ptr.hasNext()){
			System.out.print(ptr.next()+" ");
		}
		list=this.preOrder(this.root);
		ptr=list.iterator();
		System.out.println("\nPreOrder:");
		while(ptr.hasNext()){
			System.out.print(ptr.next()+" ");
		}
		list=this.postOrder(this.root);
		ptr=list.iterator();
		System.out.println("\nPostOrder:");
		while(ptr.hasNext()){
			System.out.print(ptr.next()+" ");
