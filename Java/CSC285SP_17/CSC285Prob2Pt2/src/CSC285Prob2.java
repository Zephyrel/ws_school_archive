...
public class CSC285Prob2{
	public static void main(String[] args){
		...
		StudentClassManager<Student> AcademicClass = new StudentClassManager<Student>();
		...
		printRecords(AcademicClass);
		//Below: Part 3
		StudentClassManager<Double> doubleList = new StudentClassManager<Double>();
		double set[] = {2.3,1.5,15.6,0.9,11.2,8.9,12.4,51.2,-3.5};
		for(int i=0;i<set.length;i++){
			doubleList.add(set[i]);
		}
		SortAscDouble(doubleList);
		System.out.println("\n\nPrinting a sorted list of doubles");
		for(int i=0;i<set.length;i++){
			System.out.println(doubleList.get(i));
		}
	}
	public static void SortAscDouble( ArrayList<Double> AcademicClass){	//DONE
		//Bubble Sort Ascending (Greatest to Least)
		for(int i=0;i<AcademicClass.size()-1;i++){
			for(int j=1;j<AcademicClass.size()-i;j++){
				Double tmp = AcademicClass.get(j-1);
				Double tmp2 = AcademicClass.get(j);
				if(tmp<tmp2){
...
class StudentClassManager<T> extends ArrayList<T>{
	ArrayList<T> StudentClassManager = new ArrayList<T>();
}