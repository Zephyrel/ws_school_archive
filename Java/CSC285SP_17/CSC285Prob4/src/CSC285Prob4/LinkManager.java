...
public class LinkManager<T> implements Iterable<T>{
	...
	public void addIn(int index, T t){	//DONE
		...
		int i=0;
		while(pointer.hasNext() && i!=index){pointer.next();i++;}
		pointer.add(t);
	}
	...
	public T get(int index){
		...
		ListIterator<T> pointer = iterator();
		int i=0;
		while(i!=index&&i<size()){pointer.next();i++;}
		return pointer.next();
	}
	public T getFirst(){return head.data;}
	public T getLast(){return tail.data;}
	//retrieve and removes head
	public T poll(){return removeFirst();}
	public T pollFirst(){if(size==0)return null;else{return removeFirst();}}
	public T pollLast()	{if(size==0)return null;else{return removeLast();}}
	...
	public T peek(int index){
		...
		ListIterator<T> pointer = iterator();
		do{pointer.next();}while(index!=pointer.previousIndex());
