...
public class CSC285Prob2{
	...
	public static void AddStudent( LinkedList<Student> AcademicClass, Student Obj){	//This is a useless method but on the assignment.
		...
		if(AcademicClass.isEmpty()){
			AcademicClass.add(Obj);
		}
		boolean inserted = false;
		for(int i=0;i<=AcademicClass.size() && inserted==false;i++){
			if((Obj.compareTo(AcademicClass.get(i))==-1)&&(Obj.compareTo(AcademicClass.get(i+1))==-1)){
				continue;
			}else if((Obj.compareTo(AcademicClass.get(i))==-1)&&(Obj.compareTo(AcademicClass.get(i+1))==0)){
				AcademicClass.add(i, Obj);
				inserted = true;
			}else if((Obj.compareTo(AcademicClass.get(i))==-1)&&(Obj.compareTo(AcademicClass.get(i+1))==-1)){
...
class Student implements Comparable<Student>{
	...
	public int compareTo(Student a){
		...
		if((this.getAverage()-a.getAverage())>0.0){
			i=1;
		}else if(this.getAverage()-a.getAverage()<0.0){
			i=-1;
		}else{
			i=0;
		}
